...
// update dirs (annotate placeholders, add learnings) and review-and-fix
// dirs (ancestors, where the change may have invalidated learnings —
// fixing or deleting them is explicitly licensed and must be accounted
// for in the reply). A "manual" job is a reconciliation with no change
// record: one general review section, plus any lint-flagged references.
...
`)
		...
		b.WriteString("\n3. AGENTS.md: add or refine 1–5 brief learnings inside the markers that would help an agent working in this directory, based on your review. Prefix each new learning with \"(manual)\". ")
...
`, job.Change, job.Title, dirs.String())
	b.WriteString(gardenerStructureStep)
	fmt.Fprintf(&b, "\n3. AGENTS.md: add or refine 1–5 brief learnings inside the markers that would help an agent working in this directory, based on THIS change. Prefix each new learning with the change ID in parentheses, e.g. \"(%[1]s)\". ", job.Change)
	b.WriteString(gardenerHardRules)
	b.WriteString("\n")
	b.WriteString(gardenerScopeStep)
...
- Add a learning only if the change taught something at that directory's level (conventions or facts spanning its children); new learnings carry the (%[2]s) prefix like anywhere else.
