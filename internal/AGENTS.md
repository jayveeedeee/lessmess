...
- (manual) `.lessmess/` tooling state is split one file per feature, all written via `model.WriteFileAtomic`: `sessions.json` (session mapping), `docs-queue.json`, `docs-seed.json`, `settings.json`, `settings-change.json`, and `xdg/opencode/cli.json`; follow that pattern for new server-side state rather than adding a shared grab-bag file.
<!-- tasktracker:end -->
