...
| --- | --- | --- | --- | --- | --- |
| [PSB-00](tasks/00-spike-subagent-session-basics.md) | Spike: subagent child promptability and ID visibility | Test | — | 2026-09-15 | Q1 YES (prompted finished explore child, replied OK under its agent config). Q2 NO (parent model never sees child ID). Title==description confirmed. Evidence in task notes. |
| [PSB-01](tasks/01-mapping-task-fields.md) | Mapping schema: task and parent fields | Test | — | 2026-09-15 | `go vet` clean; full `go test ./...` green. Old-shaped files load without empty keys; new fields round-trip; listByTask verified; client parses parentID. |
| [PSB-02](tasks/02-bind-endpoint.md) | Bind endpoint for task sessions | Test | PSB-00, PSB-01 | 2026-09-15 | `go vet` clean; full suite green. Guard matrix tested: 201 happy (Parent from live session), 200 idempotent retry, 409 rebind/wrong-change caller/taken sub, 422 unknown task + dead sub, 404 unknown change, 503 no service. |
| [PSB-03](tasks/03-parentid-reconciler.md) | parentID reconciler on session list | Test | PSB-01 | 2026-09-15 | `go vet` clean; full suite green. Children of bound sessions map on list serve: task from title prefix (unknown/absent prefixes map taskless), deduped across serves, foreign parents ignored, service failure fail-open. |
| [PSB-04](tasks/04-change-prompt-delegation.md) | changePrompt delegation and bind instructions | Test | PSB-00, PSB-02 | 2026-09-15 | `go vet` clean; full suite green; prompt pins extended (Delegation is optional, description prefix, title verbatim). The new prime reaches only sessions created after the 9090 server restart (user step). |
| [PSB-05](tasks/05-board-sub-list.md) | Board UI: per-task sub list with talk button | Test | PSB-00, PSB-01, PSB-02, PSB-03 | 2026-09-15 | `go vet` + full suite green; binary rebuilt. Live (scratch repo, port 9099): board HTML serves #board-subs, task-sessions bind 201, sessions payload carries task/parent, served app.js has the chip renderer. Browser-visual check awaits the user's 9090 server restart. |
