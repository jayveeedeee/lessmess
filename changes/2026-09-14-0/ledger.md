...
| --- | --- | --- | --- | --- | --- |
| [SPS-00](tasks/00-sticky-header.md) | Sticky top menu bar | Test | — | 2026-09-14 | Build+vet+test pass; rebuilt binary serves sticky header rule (app.css served line 86); overlay/detail z-order unchanged by inspection; full visual pass in SPS-03 |
| [SPS-01](tasks/01-scope-strip.md) | Settings scope strip with segmented control | Test | SPS-00 | 2026-09-14 | Strip served with radios/values/checked intact; server tests green (zero test edits); page-width caps removed early because the flush strip needs full width (was SPS-02 scope) |
| [SPS-02](tasks/02-split-shell.md) | Settings split-shell layout and content de-clutter | Test | SPS-01 | 2026-09-14 | Split shell served; h1/nav/footer in sidebar, error+hint atop content, all five sections unchanged; every render-test string present in served HTML; full go test green; zero app.js edits |
| [SPS-03](tasks/03-verification.md) | End-to-end verification and docs update | Test | SPS-02 | 2026-09-14 | Full suite green (`-count=1`), validate exit 0 after root-ledger sync, dead-CSS sweep done, layering audit ok, zero test edits; user visual pass pending (needs server restart) |
| [SPS-04](tasks/04-general-section.md) | General settings group for wizard re-entry | Test | SPS-02 | 2026-09-14 | General nav item + section live, wizard ghost-button moved from footer (footer keeps layer explanation); server tests + validate green; 9090 restarted with new build |
| [SPS-05](tasks/05-page-footer.md) | Page-wide settings footer for layer explanation | Test | SPS-02 | 2026-09-14 | Footer spans page width under the split; sidebar footer removed (sidebar = title + nav); test strings intact; tests + validate green; 9090 restarted |
| [SPS-06](tasks/06-pinned-footer-flat-nav.md) | Pinned footer and flat full-width side-menu selections | Test | SPS-05 | 2026-09-14 | Full-height app layout (board pattern), footer pinned flush bottom, nav hover+active full-width square (name-pill idiom dropped), mobile fallback restores page scroll; CSS-only, tests + validate green, 9090 restarted |
